...
export default class ScoreFacotrs extends Component {
    ...
    get_recent_contact = async() => {
        ...
        .then(async data => {
            ...
            console.log(JSON.stringify(chat_recent_list))
        })
        .catch(function(error) {
        ...
        if(button_type == "ok") {
            ...
            .then(response => {
                ...
                if(status_code == 200) {
                    ...
                    Alert.alert('EpatientIndex', "There is an error to request. Please try again.");
                }
            })
            .catch(function(error) {
                Alert.alert('Warning!', "Network error.");
    ...
    request_accept = async(accept_type, index) => {
        ...
        await fetch(url, {
            ...
            body: JSON.stringify({
                id: 0,
                userContact: this.state.chat_request_list[index].userContact,
                userName: Global.user_name
            })
        })
        .then(response => {
            console.log(response)
            console.log(Global.user_name + "/////////////////////" + this.state.chat_request_list[index].userContact)
            const status_code = response.status;
            if (status_code == 200) {
    ...
    render() {
        ...
        <SafeAreaView style = {styles.container}>
        {
            ...
            <View style = {{position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, justifyContent: 'center', alignItems: 'center', backgroundColor: 'black', opacity: 0.3, zIndex: 200}}>
                <View style = {{flex: 1}}>
        ...
        {
            this.state.show_right_menu_view &&
            <View style = {{width: deviceWidth, height: deviceHeight, left: 0, right: 0, position: 'absolute', zIndex: 10}} onStartShouldSetResponder={() => this.setState({show_right_menu_view: false})}/>
        }
        {
            ...
            <View style = {{position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, justifyContent: 'center', alignItems: 'center', zIndex: 100}}>
                <View style = {styles.chat_request_view}>
                    ...
                    <View style = {styles.chat_request_id_view}>
                        <TextInput style = {styles.id_text_input} autoCapitalize='none' onChangeText = {(text) => this.setState({chat_request_id: text})}>{this.state.chat_request_id}</TextInput>
                    </View>
